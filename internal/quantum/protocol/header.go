...
type Header struct {
	...
	GUUID          guuid.UUID  // 16 bytes - Global unique identifier
...
func NewHeader(guid guuid.UUID, seqNum, ackNum uint32, flags Flags) *Header {
	return &Header{
...
func (h *Header) String() string {
	guidStr := h.GUUID.String()
	if len(guidStr) > 8 {
		guidStr = guidStr[:8]
	}
	return fmt.Sprintf("Quantum{GUUID:%s, Seq:%d, Ack:%d, Flags:0x%02X, PayloadLen:%d, SACKBlocks:%d}",
		guidStr, h.SequenceNumber, h.AckNumber, uint8(h.Flags), h.PayloadLength, len(h.SACKBlocks))
}
